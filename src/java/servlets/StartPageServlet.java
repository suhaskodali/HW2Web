...
public class StartPageServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            List<MusicVoteBean> listOfBeans = (List<MusicVoteBean>) request.getSession().getAttribute("listOfBeans");
            for (int i=0; i<listOfBeans.size(); i++) {
                out.println(listOfBeans.get(i).getMusicType() + " has " + listOfBeans.get(i).getNumVotes() + " votes </br>");
